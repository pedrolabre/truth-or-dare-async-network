...
1\. Criação de truth via API refletida no feed



2\. Manutenção do contrato do feed após criação dinâmica de truths
...
&#x20;   √ deve refletir no feed uma truth criada via API (519 ms)

&#x20;   √ deve manter o contrato do feed ao criar novas truths dinamicamente (174 ms)
...
\- uma truth criada via API é imediatamente refletida no feed
...
A suíte `truths.integration.test.ts` valida com sucesso a integração entre criação de conteúdo e consumo no feed, garantindo consistência entre as operações de escrita e leitura e assegurando que o backend suporta corretamente o fluxo real do produto.
